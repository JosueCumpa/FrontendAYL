...
import AgregarCamiones from "../Camion/AgregarCamiones";
import EditarCamiones from "../Camion/EditarCamiones";
import EliminarCamiones from "../Camion/EliminarCamiones";
  ...
  const list = useAsyncList({
    async load({ signal }) {
      try {
        ...
        const res = await axios.get(`${API_BASE_URL}/Camion/`, {
          headers: {
        ...
        const filteredItems = searchTerm
          ? res.data.filter(
              (item) =>
                ...
                item.nombre_apellido_conductor
                  .toLowerCase()
        ...
        if (error.response && error.response.status === 401) {
          try {
            ...
            const refreshedRes = await axios.get(`${API_BASE_URL}/Camion/`, {
              headers: {
      ...
      >
        <CardHeader className="flex gap-3 justify-start font-bold text-large ">
          <div className=" px-8 rounded-2xl flex gap-3  grid grid-cols-2 sm:grid-cols-4">
            <span style={{ paddingTop: "10px" }}>GESTION DE CAMIONES</span>
            {agregar && (
              <AgregarCamiones
              ...
              <TableRow key={item?.id}>
                ...
                <TableCell>
                  {item?.fecha_actualizacion &&
                    format(new Date(item?.fecha_actualizacion), "yyyy-MM-dd")}
                </TableCell>
                <TableCell>{item?.nombre_apellido_conductor}</TableCell>
                <TableCell>
                  <Chip
