...
import AgregarGrifo from "../grifo/AgregarGrifo";
import EditarGrifo from "../grifo/EditarGrifo";
import EliminarGrifo from "../grifo/EliminarGrifo";
      ...
      try {
        ...
        if (error.response && error.response.status === 401) {
          try {
            ...
            const refreshedRes = await axios.get(`${API_BASE_URL}/Grifo/`, {
              headers: {
      ...
      >
        <CardHeader className="flex gap-3 justify-start font-bold text-large ">
          <div className=" px-8 rounded-2xl flex gap-3  grid grid-cols-2 sm:grid-cols-4">
            <span style={{ paddingTop: "10px" }}>GESTION DE GRIFOS</span>
            {agregar && (
              <AgregarGrifo
                style={{ paddingTop: "10px" }}
                onClose={() => setAgregar(false)}
                updateGrifo={updateGrifo}
              />
            )}
