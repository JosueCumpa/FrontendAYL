...
export default function EditarBanco({ Banco, type = "edit", updateBancos }) {
  ...
  const handleSubmit = async (onClose) => {
    ...
    if (!isnombreValid) {
      ...
      try {
        if (type === "edit") {
          const url = `${API_BASE_URL}/Banco/${Banco?.id}/`;
          ...
          await toast.promise(axios.patch(url, data, { headers }), {
            ...
            error: "Error al actualizar el Banco! Verifique los campos",
          });
        }
  ...
  const handleOpen = (onOpen) => {
    if (type == "edit") {
      setEstate(Banco.estado);
      setNombre(Banco.nombre);
      setAbreviatura(Banco.abreviatura);
    }
    onOpen();
  };
          ...
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                {type === "add" ? "Agregar Banco" : "Editar Banco"}
              </ModalHeader>
              <ModalBody>
                <div className="flex relative flex-col gap-4">
                  ...
                  <Input
                    ...
                    isInvalid={error}
                    errorMessage={
                      error && "Ingrese la abreviatura correctamente"
                    }
                    value={abreviatura}
                    onChange={(e) => handleNameChange(e, setAbreviatura)}
                  ></Input>
